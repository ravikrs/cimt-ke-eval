package de.rwth.i9.cimt.ke.eval.util;
...
public class WordCount extends Pair<String, Double> {
	...
	public static List<WordCount> parseIntoList(String setStr) {
		...
		List<WordCount> set = new ArrayList<WordCount>();
	...
	public static String formatIntoString(List<WordCount> set) {
		if (set == null)
			return null;
		String res = "[";
		for (WordCount wc : set) {
			res += "(" + wc.getX() + ", " + wc.getY() + "),";
		}
